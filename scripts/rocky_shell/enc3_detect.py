...
def L(p):
    ...
    return m
A=L(SP+"/leg3_aligned_3A.stl"); B=L(SP+"/leg3_aligned_3B.stl")
...
json.dump({"centerline":[round(cy,3),round(cz,3)],"report":report},open(SP+"/_enc3_report.json","w"),indent=1)
np.save(SP+"/_shell3_rout.npy",{"r_out":r_out,"cy":cy,"cz":cz,"NX":NX,"NT":NT})
print("\nsaved _enc3_report.json + _shell3_rout.npy")
