"""LEG2 per-finger grip GLOVES v2 — trimesh + manifold3d build (operator correction).
...
Outer crag matched to leg2 stone (freq 0.09/mm, amp 2.5, 4 octaves).
"""
import json, math
import numpy as np
import trimesh
...
# ---------------- numpy value noise (leg2 stone family: freq 0.09, 4 octaves) ---
...
def outer_prim(ax, seed, prong_mesh):
    ...
    follows the toy 2-B prong surface (registered/scaled to this finger) wherever
...
def build_prim(tag, f_self, f_other, ax, seed, keep_pos_z):
    ...
    pp = CD + f"/_prong_{tag}.stl"
    if os.path.exists(pp):
        prong = trimesh.load(pp, process=True)
    else:
        print(f"  [bootstrap] no _prong_{tag}.stl yet -> fallback-crag outer only")
        prong = trimesh.Trimesh(np.array([[0,0,-9e3],[1,0,-9e3],[0,1,-9e3]]),
                                np.array([[0,1,2]]), process=False)  # far away: never governs
