...
mA=to_st_mesh("2-A"); mB=to_st_mesh("2-B")
...
# assign by proximity: HIP end of 2-A is the one FAR from 2-B; KNEE end near 2-B
...
# 2-B: knee end near kneeA; toe end far
...
for nm,P,tgt in [("HIP",apply(RA,sA,tA,hip),[0,0,0]),
                 ("KNEE(2A)",apply(RA,sA,tA,kneeA),[158,0,0]),
                 ("KNEE(2B)",apply(RB,sB,tB,kneeB),[158,0,0]),
                 ("TOE",apply(RB,sB,tB,toe),[328,0,0])]:
    P=P[0]; print(f"{nm:9s} -> {np.round(P,2)}  target {tgt}  resid {np.round(P-np.array(tgt),3)}")
print(f"\nscale 2-A sA={sA:.3f}  (hip->knee {LA:.2f} -> 158)")
print(f"scale 2-B sB={sB:.3f}  (knee->toe {LB:.2f} -> 170)")
# knee bend / extension
...
# component perpendicular to X (the roll indicator)
def roll_of(z):
    zp=z-z[0]*np.array([1,0,0]); return np.degrees(np.arctan2(zp[2],zp[1]))
print(f"roll-frame (atan2 of mapped o3d-Z about X): 2-A={roll_of(zA):.1f} deg, 2-B={roll_of(zB):.1f} deg, A-B offset={roll_of(zA)-roll_of(zB):.1f} deg")

# ---- export aligned meshes (chassis frame) ----
mA2=mA.copy(); mA2.vertices=apply(RA,sA,tA,mA.vertices); mA2.export(SP+"/leg2_aligned_2A.stl")
mB2=mB.copy(); mB2.vertices=apply(RB,sB,tB,mB.vertices); mB2.export(SP+"/leg2_aligned_2B.stl")
print("\nexported leg2_aligned_2A.stl / _2B.stl  bboxA",np.round(mA2.bounds,1).tolist(),"bboxB",np.round(mB2.bounds,1).tolist())

def M(R,s,t): 
    m=np.eye(4); m[:3,:3]=s*R; m[:3,3]=t; return m.tolist()
out={
 "description":"LEG2 align-only: assembled o3d leg (2-A+2-B, pegs kept), extended+aligned to D-043 chassis skeleton (HIP=0,KNEE=158,TOE=328 along +X).",
 ...
 "o3d_joints_statue_mm":{"HIP":hip.tolist(),"KNEE_2A":kneeA.tolist(),"KNEE_2B":kneeB.tolist(),"TOE":toe.tolist(),
                         "knee_ball_socket_gap_mm":float(np.linalg.norm(kneeA-kneeB))},
 "scale":{"2-A":sA,"2-B":sB},
 "source_span_mm":{"2-A_hip_knee":LA,"2-B_knee_toe":LB},
 "knee_extension_deg":float(180-ang),
 "o3d_interior_knee_angle_deg":float(ang),
 "transforms_o3dstatue_to_chassis_4x4":{"2-A":M(RA,sA,tA),"2-B":M(RB,sB,tB)},
 "note":"NOTHING trimmed/hollowed/anchored. Pegs kept. Align+show only, for operator confirmation."
}
json.dump(out,open(SP+"/leg2_aligned.json","w"),indent=1)
np.save(SP+"/_leg2_final_joints.npy",{"hip":hip,"kneeA":kneeA,"kneeB":kneeB,"toe":toe,
      "RA":RA,"sA":sA,"tA":tA,"RB":RB,"sB":sB,"tB":tB},allow_pickle=True)
print("wrote leg2_aligned.json")
