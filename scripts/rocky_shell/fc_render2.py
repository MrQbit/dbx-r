"""Render leg2 v2 snap-fit finger gloves for operator review.
...
OrigB = imp(SP + "/leg2_aligned_2B.stl"); OrigB.data.materials.append(stone)
A = imp(SP + "/leg2_enclosed_2A.stl"); A.data.materials.append(stone)
B = imp(SP + "/leg2_enclosed_2B.stl"); B.data.materials.append(stone)
TO = imp(SP + "/leg2_enclosed_toecover.stl"); TO.data.materials.append(toe_mat)
...
for nm, frm in [("SIDE", (175, -760, 20)), ("TOP", (175, 40, 760)), ("3Q", (560, -560, 360))]:
    look(frm, FULLC, 400); render(SP + f"/_fc_full_{nm}.png")

# 2) grip close-up solid
show(shell + [cA, cB, cTo])
look(GCAM, GC, GSC); render(SP + "/_fc_closeup_solid.png")
