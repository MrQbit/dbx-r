...
CY,CZ=1.56,4.20          # shell centerline offset (from detection)
...
def verts_np(o):
    ...
    v=(v@M[:3,:3].T)+M[:3,3]
    return v
def outward_face_pts(o):
    """Face centroids that face radially OUTWARD (about CY,CZ). [LEG3] the fork
    window's slot walls face across the slot, not outward — excluding them stops
    the push field from treating a slot wall as 'shell too thin' and blowing
    both branches into fins. Solid regions are unaffected."""
    m=o.data
    nf=len(m.polygons)
    c=np.empty(nf*3); m.polygons.foreach_get("center",c); c=c.reshape(-1,3)
    nrm=np.empty(nf*3); m.polygons.foreach_get("normal",nrm); nrm=nrm.reshape(-1,3)
    M=np.array(o.matrix_world)
    c=(c@M[:3,:3].T)+M[:3,3]; nrm=nrm@M[:3,:3].T
    ry=c[:,1]-CY; rz=c[:,2]-CZ
    rr=np.hypot(ry,rz); rr[rr<1e-6]=1
    rad=(nrm[:,1]*ry+nrm[:,2]*rz)/rr
    return c[rad>0.1]

A=imp(SP+"/leg3_aligned_3A.stl"); A.name="shellA"
B=imp(SP+"/leg3_aligned_3B.stl"); B.name="shellB"
...
shell_v=np.vstack([outward_face_pts(A),outward_face_pts(B)])   # [LEG3] outward-facing only
Rsh_min,cnt=grid_reduce(shell_v,None,"p10")
Rsh_max,_=grid_reduce(np.vstack([av,bv]),None,"max")
...
xt=smooth(46,58,xcent)*(1-smooth(330,342,xcent))   # LEG3: full push by x=58 (leg3 roll leaves femur yoke proud at x~60 under the 56..70 ramp)
push*=xt[:,None]

# blur push over (x,theta), theta wraps
def blur(g,sx=2.4,st=2.4):   # [LEG3] wider falloff: pushes are 2x leg2's (slim forked shell), 1.4 left knife-edge fins
    from math import exp
    kx=np.array([exp(-(i*i)/(2*sx*sx)) for i in range(-6,7)]); kx/=kx.sum()
    kt=np.array([exp(-(i*i)/(2*st*st)) for i in range(-6,7)]); kt/=kt.sum()
    out=g.copy()
    tmp=np.zeros_like(g)
    for i,w in enumerate(kx):
        tmp+=w*np.roll(g,i-6,axis=0)
    out2=np.zeros_like(g)
    for i,w in enumerate(kt):
        out2+=w*np.roll(tmp,i-6,axis=1)   # theta wrap via roll
    return out2
# LEG3: smooth envelope >= requirement. Plain blur attenuates peaks (1-9mm leaks
# on this slimmer shell); raw fmax tears the mesh (C0 kinks at bin edges).
# Iterative blurred-residual completion stays smooth (sum of gaussians) while
# closing the gap to the raw requirement.
pushb=blur(push)
for _ in range(4):
    pushb=pushb+blur(np.clip(push-pushb,0,None))
...
export(A,SP+"/leg3_enclosed_3A.stl")
export(B,SP+"/leg3_enclosed_3B.stl")
export(toe,SP+"/leg3_enclosed_toecover.stl")
