"""LEG4 toe piece v2 — REAL toy 4-B foot-tip sculpt re-purposed as the toe/ankle
cosmetic (mirror of the operator-approved leg2 recipe, fc_toe_build.py).

LEG4-SPECIFIC (flagged): 4-B's natural 2-prong cleft is ROLLED ~90 deg vs 2-B —
groove opens +z with split plane y~7.2 (2-B: opened +y, split plane z~2.5).
The extracted foot region is pre-rotated Rx(-90) about the groove center
((y,z)->(z,-y)) so the cleft lands on z=0 opening +y exactly as the approved
leg2 recipe places it (fingers emerge in the cleft). Measured in the rotated
frame: GROOVE_Y'=0.8 (distal centroid), GROOVE_Z'=-7.2 (groove plane).
RAD_K retuned 1.37->1.12: 4-B's foot is ~1.5x chunkier about its groove; 1.12
maps it onto the same collar/mouth envelope as leg2's mapped foot.

Transform: anisotropic stretch (axial 0.91 about the tip, radial 1.12 about the
groove center) so the REAL surface swallows the chassis foot core + O52 collar +
wrist. Carves (all + clearance): chassis foot-core/wrist/palm +1.5, primary
gloves +0.8, the THUMB SWEPT VOLUME (13 poses grip 0->2.2) +1.2.
...
PRONGS_ONLY=1 env: stop after prong export (bootstrap: gloves need prongs,
toe carve needs the finished gloves).
"""
import json, math, os
import numpy as np
import trimesh
...
PRONGS_ONLY = os.environ.get("PRONGS_ONLY", "0") == "1"

AX_K, RAD_K = 0.91, 1.12     # near-NATURAL axial; radial retuned for 4-B (leg2: 1.37)
SRC_TIP, DST_TIP = 339.5, 380.0
GROOVE_Y, GROOVE_Z = 0.8, -7.2   # in the Rx(-90) rotated source frame
MOUTH_X = 330.0              # the enclosed 4B shell itself covers to ~331 (r~30)
...
full = trimesh.load(SP + "/leg4_aligned_4B.stl", process=True)
fm = full.vertices[full.faces].mean(axis=1)
region = trimesh.Trimesh(full.vertices, full.faces[fm[:, 0] > 279], process=True)
# LEG4: pre-rotate Rx(-90): (y,z)->(z,-y) so the cleft matches leg2's placement
Vr = region.vertices.copy()
region = trimesh.Trimesh(np.c_[Vr[:, 0], Vr[:, 2], -Vr[:, 1]], region.faces, process=True)
...
# from the SOURCE region (pre-anisotropy, rotated frame): prongs at src x>318,
# split by the groove plane z'=GROOVE_Z
AX_F = {"A": np.array([0.9272, 0.3664, +0.0779]), "B": np.array([0.9272, 0.3664, -0.0779])}
ROOTF = np.array([369.0, 7.0, 0.0])
# LEG4 (flagged): the shortest-arc long-axis registration leaves the prong's ROLL
# about the finger axis free; leg2's rolls happened to land over the blade
# (centroid azimuth A=-139.6deg, B=+168.8deg). Roll-align leg4's prongs to those
# approved poses so the graft governs the outer the same way.
AZI_TGT = {"A": -139.6, "B": 168.8}
# LEG4 (flagged): 4-B's second (side) prong is shorter than 2-B's; its graft
# window extends proximally (src x>306 vs leg2's 318) so the registered shard
# spans the finger like leg2's did (blade coverage 33% vs 12% at 318).
XLO = {"A": 318.0, "B": 306.0}
fm0 = region.vertices[region.faces].mean(axis=1)
for tag, sgn in (("A", +1), ("B", -1)):
    keep = (fm0[:, 0] > XLO[tag]) & (sgn * (fm0[:, 2] - GROOVE_Z) > 0.5)
    ...
    # roll-align about afin to the approved leg2 pose (see AZI_TGT note)
    up = np.array([0, 0, 1.0]); e1 = np.cross(afin, up); e1 /= np.linalg.norm(e1)
    e2 = np.cross(afin, e1)
    ss = V1 @ afin
    band = (ss > ss.min() + 0.3 * (ss.max() - ss.min())) & (ss < ss.min() + 0.8 * (ss.max() - ss.min()))
    Wb = V1[band] - np.outer(V1[band] @ afin, afin)
    azi = float(np.degrees(np.angle(np.mean(np.exp(1j * np.arctan2(Wb @ e2, Wb @ e1))))))
    dphi = math.radians(AZI_TGT[tag] - azi)
    ca, sa = math.cos(dphi), math.sin(dphi)
    Kx = np.array([[0, -afin[2], afin[1]], [afin[2], 0, -afin[0]], [-afin[1], afin[0], 0]])
    Rr = np.eye(3) + sa * Kx + (1 - ca) * (Kx @ Kx)
    V1 = V1 @ Rr.T
    print(f"  prong {tag}: roll-aligned {azi:.1f} -> {AZI_TGT[tag]:.1f} deg about finger axis")
    smin = (V1 @ afin).min()
...
if PRONGS_ONLY:
    print("PRONGS_ONLY: stopping before carve (gloves not built yet)")
    raise SystemExit(0)
...
mm = stl_jitter_export(toe, SP + "/leg4_enclosed_toecover.stl")
lo, hi = mm.bounds
