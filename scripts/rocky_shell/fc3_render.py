"""Render LEG3 complete-leg panels for operator review (mirrors leg2's fc_render2.py
+ enc_render ghost panels). Panels: full-leg solid (SIDE/TOP/3Q), ghost chassis,
grip close-up solid + ghost, open/closed articulation, 2 cross-sections, and the
comparison original toy 3-B tip vs ours."""
...
ch_ghost = mat("chg", (0.30, 0.60, 0.95), 0.5, emis=0.15)
hip_flag = mat("hip", (0.95, 0.35, 0.12), 0.5, emis=0.5)
digit_flag = mat("dig", (0.95, 0.75, 0.15), 0.5, emis=0.35)

OrigB = imp(SP + "/leg3_aligned_3B.stl"); OrigB.data.materials.append(stone)
A = imp(SP + "/leg3_enclosed_3A.stl"); A.data.materials.append(stone)
B = imp(SP + "/leg3_enclosed_3B.stl"); B.data.materials.append(stone)
TO = imp(SP + "/leg3_enclosed_toecover.stl"); TO.data.materials.append(toe_mat)
...
# chassis (ghost row)
frame = imp(BP + "/leg_chassis_neutral_frame.stl"); frame.data.materials.append(ch_ghost)
servo = imp(BP + "/leg_chassis_neutral_servos.stl"); servo.data.materials.append(hip_flag)
hand  = imp(BP + "/leg_chassis_neutral_hand.stl");  hand.data.materials.append(digit_flag)
chassis = [frame, servo, hand]
...
ALL = shell + fingers + covers + [secA, secT, OrigB] + chassis
def show(objs):
...
for nm, frm in [("SIDE", (175, -760, 20)), ("TOP", (175, 40, 760)), ("3Q", (560, -560, 360))]:
    look(frm, FULLC, 400); render(SP + f"/_fc3_full_{nm}.png")

# 1b) ghost chassis inside (SIDE + 3Q)
for o in shell: setmat(o, stone_g)
for c in covers: setmat(c, cover_g)
show(shell + [cA, cB, cTo] + chassis)
look((175, -760, 20), FULLC, 400); render(SP + "/_fc3_ghost_SIDE.png")
look((560, -560, 360), FULLC, 400); render(SP + "/_fc3_ghost_3Q.png")

# 2) grip close-up solid
for o in shell: setmat(o, stone if o != TO else toe_mat)
for c in covers: setmat(c, stone_c)
show(shell + [cA, cB, cTo])
look(GCAM, GC, GSC); render(SP + "/_fc3_closeup_solid.png")

# 3) ghost: coloured fingers inside translucent gloves
for o in shell: setmat(o, stone_g)
for c in covers: setmat(c, cover_g)
show(shell + [fA, fB, fTo] + [cA, cB, cTo])
look(GCAM, GC, GSC); render(SP + "/_fc3_closeup_ghost.png")

# 4) articulation open/closed
for o in shell: setmat(o, stone_g)
for c in covers: setmat(c, stone_c)
show([fA, fB, fTo, cA, cB, cTo])
look(GCAM, GC, GSC); render(SP + "/_fc3_open.png")
show([fA, fB, fTc, cA, cB, cTc])
look(GCAM, GC, GSC); render(SP + "/_fc3_closed.png")

# 4b) COMPARISON: original toy 3-B foot tip vs our covered tip (same view dir)
for o in shell: setmat(o, stone if o != TO else toe_mat)
show([OrigB])
look((315 + 180, -170, 130), (312, 0, 3), 110)
render(SP + "/_fc3_compare_orig.png")
show(shell + [cA, cB, cTo])
look((358 + 180, -170, 130), (360, 0, -2), 138)
render(SP + "/_fc3_compare_ours.png")

# 5) sections: cavity + wall visible, finger shown inside
show([secA, fA])
look((root + mvec * 220 + Vector((25, 0, 70))), (391, 1, 2), 70)
render(SP + "/_fc3_section_prim.png")
show([secT, fTo])
look((366, -35, 220), (366, -35, 0), 75)
render(SP + "/_fc3_section_thumb.png")
print("RENDER_DONE")
