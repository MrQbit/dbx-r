"""LEG5 toe piece — REAL toy 5-B foot-tip sculpt re-purposed as the toe/ankle
cosmetic (operator-approved leg2 recipe, puppet principle: derive tip from the
ORIGINAL geometry).

Source: leg5_aligned_5B.stl = the o3d-aligned toy tibia+foot in the chassis frame
(scale 4.474 — the 5-B LONG BLADE foot, criss-cross engravings; leg2's 2-B was
scale 6.0). Its distal taper x[278..340] IS the toy blade tip: a broad flat
spade with striated engravings (no fork cleft like 2-B — the prong split for the
glove graft is the blade midplane at the tip centroid z=-0.89 instead).

Transform: anisotropic stretch (axial 0.95 near-NATURAL about the tip, radial
1.50 about the tip centroid) so the REAL surface spans x[~321..380], swallowing
the chassis foot core + collar + wrist. Radial functional floor r>=28.8 only in
the collar band x[330..348] (reported).
...
Also exports the two scaled REAL BLADE-HALF surfaces for the glove outer graft.

PHASES (argv[1]): 'prongs' = region + prong export only (pre-covers);
                  'full'   = everything incl. boot carve (post-covers).
"""
...
AX_K, RAD_K = 0.95, 1.50     # near-NATURAL axial; radial meets the collar floor
SRC_TIP, DST_TIP = 340.0, 380.0
GROOVE_Y, GROOVE_Z = 0.76, -0.89   # 5-B tip centroid (blade midplane split)
MOUTH_X = 330.0              # the enclosed 5B shell itself covers to ~332 (r~30)
...
full = trimesh.load(SP + "/leg5_aligned_5B.stl", process=True)
fm = full.vertices[full.faces].mean(axis=1)
region = trimesh.Trimesh(full.vertices, full.faces[fm[:, 0] > 278], process=True)
...
# ---------- 4) REAL BLADE-HALF surfaces registered to the fingers (glove graft) --
# from the SOURCE region (pre-anisotropy): distal 22mm of the blade, split by the
# blade midplane (z=GROOVE_Z) — 5-B has no fork; each half is a THICK FLAT slab.
...
# soften the machined carve walls with light matched crag (keeps >=0.8 clearances)
...
mm = stl_jitter_export(toe, SP + "/leg5_enclosed_toecover.stl")
lo, hi = mm.bounds
print("TOE leg5: dims %.1f x %.1f x %.1f vol=%.0f" % (hi[0]-lo[0], hi[1]-lo[1], hi[2]-lo[2], mm.volume))
print("DONE toe leg5 (real sculpt surface, %.1f%% floor-adjusted)" % floored)
