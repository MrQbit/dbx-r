"""LEG5 per-finger grip GLOVES v2 — trimesh + manifold3d build (operator correction).

3 snap-fit covers for the real 2+1 grip:
  * 2 PRIMARY covers: THICK + FLAT craggy blades (toy 5-B's flat blade tip, midplane-split halves). The
...
Outer crag matched to leg5 stone (freq 0.09/mm, amp 2.5, 4 octaves).
"""
import json, math
import numpy as np
import trimesh
...
# ---------------- numpy value noise (leg5 stone family: freq 0.09, 4 octaves) ---
...
def outer_prim(ax, seed, prong_mesh):
    ...
    follows the toy 5-B blade-half surface (registered/scaled to this finger) wherever
